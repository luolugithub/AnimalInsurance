...
public abstract class CameraActivity extends Activity implements OnImageAvailableListener {
    ...
    private View.OnClickListener okButtonClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (ANIMAL_TYPE == 0) {
                Toast.makeText(CameraActivity.this, "必须选择其中一个农险！！", Toast.LENGTH_SHORT).show();
                return;
            }
            setFragment();
            dialog.dismiss();
    ...
    private void requestPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (shouldShowRequestPermissionRationale(PERMISSION_CAMERA) || shouldShowRequestPermissionRationale(PERMISSION_STORAGE)) {
                Toast.makeText(CameraActivity.this, "Camera AND storage permission are required for this apply", Toast.LENGTH_LONG).show();
            }
